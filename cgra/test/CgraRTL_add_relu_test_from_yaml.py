"""
=========================================================================
CgraRTL_add_relu_test_from_yaml.py
=========================================================================
Numerical test for the compiled 32-element INT32 Add+ReLU kernel.
"""
  ...
  def construct(s, DUT, FunctionUnit, FuList,
    ...
    def cmp_fn(a, b):
      return (
        ...
        and a.payload.data.payload == b.payload.data.payload
        and a.payload.data_addr == b.payload.data_addr
      )
...
rhs_base = 32
output_base = 64
lhs = [i - 16 for i in range(num_elements)]
rhs = [8 - (i % 7) * 3 for i in range(num_elements)]
expected = [max(lhs[i] + rhs[i], 0) for i in range(num_elements)]
preload_data = [
    [
        IntraCgraPktType(
            payload = CgraPayloadType(
                CMD_STORE_REQUEST,
                data = DataType(to_uint32(lhs[i]), 1),
                data_addr = lhs_base + i))
        for i in range(num_elements)
    ] + [
        IntraCgraPktType(
            payload = CgraPayloadType(
                CMD_STORE_REQUEST,
                data = DataType(to_uint32(rhs[i]), 1),
                data_addr = rhs_base + i))
        for i in range(num_elements)
    ] + [
        IntraCgraPktType(
            payload = CgraPayloadType(
                CMD_STORE_REQUEST,
                data = DataType(0xdeadbeef, 1),
                data_addr = output_base + i))
        for i in range(num_elements)
    ]
]

def sim_add_relu(cmdline_opts, mem_access_is_combinational):
  src_ctrl_pkt = []
  complete_signal_sink_out = []
  src_query_pkt = []

  kLoopLowerBound = 0
  kLoopUpperBound = num_elements
  kCtrlCountPerIter = 5
  ...
  script_factory = ScriptFactory(path = "validation/test/add_relu.yaml",
                                    CtrlType = CtrlType,
  ...
  src_query_pkt = [
      IntraCgraPktType(payload = CgraPayloadType(
          CMD_LOAD_REQUEST, data_addr = output_base + i))
      for i in range(num_elements)
  ]

  expected_complete_sink_out_pkg = [
      IntraCgraPktType(payload = CgraPayloadType(
          CMD_COMPLETE, DataType(0, 0, 0, 0)))
      for _ in src_opt_pkt0
  ]
  expected_mem_sink_out_pkt = [
      IntraCgraPktType(payload = CgraPayloadType(
          CMD_LOAD_RESPONSE,
          data = DataType(value, 0),
          data_addr = output_base + i))
  ...
  trace_file = os.path.join(trace_dir, 'trace_add_relu_4x4_Mesh.jsonl')
  init_trace_logger(trace_file, x_tiles, y_tiles, "Mesh", cgra_id)

  cmdline_opts['max_cycles'] = 1000
...
def test_homogeneous_4x4_add_relu_combinational_mem_access(cmdline_opts):
  sim_add_relu(cmdline_opts, mem_access_is_combinational = True)
